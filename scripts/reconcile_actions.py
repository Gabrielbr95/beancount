...
reconcile_actions.py — Route, enrich and sanity-check B3 beancount extract output.
    ...
    Usage:
        python scripts/reconcile_actions.py tmp.bean
        python scripts/reconcile_actions.py tmp.bean --rewrite
"""
...
def main():
    ...
    prices_dir = base_dir / "beans" / "prices"
    ...
    append_to_file(base_dir / "beans" / "corporate_actions.bean", enriched_corp, rewrite)
    append_to_file(base_dir / "beans" / "income_events.bean", buckets["income"], rewrite)
    append_to_file(base_dir / "beans" / "transactions.bean", buckets["transactions"], rewrite)
