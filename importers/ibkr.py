...
Vendored from https://github.com/OSadovy/uabean (MIT License, Copyright (c)
Oleksii Sadovyi), file src/uabean/importers/ibkr.py. See decision [005].
"""
...
from importers.ibkr_mixins import IdentifyMixin
...
class Importer(IdentifyMixin, beangulp.Importer):
    ...
    # Note: accept either text/xml or application/xml. Python's mimetypes
    # resolves .xml to text/xml (3.14 + beangulp 0.2.0); upstream uabean's
    # environment resolves to application/xml. Content check is the real filter.
    matchers = [
        ("mime", r"(text|application)/xml"),
    ...
    def extract(self, filename, existing=None):
        ...
        return self.autoopen_accounts(transactions, existing) + transactions
