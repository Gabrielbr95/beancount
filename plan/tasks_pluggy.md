...
## Slice 7: Detect internal transfers
- [x] 25. Probe Pluggy transaction records on 2025-07-10 across all configured accounts and determine whether their fields reliably identify both sides of an internal transfer. (Verification: redacted field-level comparison captured; recommendation documented in Decision [019])
- [ ] 26. On user approval, add conservative internal-transfer pairing: require exactly one counterpart with the same posting date, currency, and absolute amount, an opposite normalized Beancount amount, and a transfer indicator; leave ambiguous candidates as independent transactions. (Verification: unit tests cover the 2025-07-10 savings transfer, a credit-card payment, and same-day equal-value ambiguity)

## Slice 8: Edit full transactions in one CSV row
- [x] 27. Reshape `scripts/ledger_csv.py` so each transaction exports as one CSV row and each posting is represented by numbered `leg_N_*` columns; rebuild postings from those columns on import. (Verification: export `tmp.bean`, confirm 1,546 CSV rows for 1,546 transactions and retain all postings after CSV → Bean round-trip; three-leg in-memory round-trip passes)
- [x] 28. Diagnose apparent transaction loss when `scripts/ledger_csv.py` exports the latest Pluggy extraction: no loss in the CSV tool. Beangulp commented out 1,548 entries as duplicates of `main.bean`; the remaining 76 active Transaction directives produced 76 CSV rows. (Verification: `loader.load_file()` found 76 Transactions in `tmp_pluggy.bean`; `tmp_pluggy.csv` contains 76 data rows)
- [!] 29. Do not add a local `PredictPostings` patch. Research found no released Smart Importer configuration or merged fix that safely supports a combined bank-and-credit-card importer; upstream issue #130 remains open and PR #153 was closed unmerged. Revisit only if the user chooses a different established import workflow or an upstream release fixes it. (Verification: research recorded in session; no local workaround deployed)
- [x] 30. Disable Smart Importer without removing it by commenting out its import and hook registration in `import.py`. (Verification: `python import.py extract --help` runs without importing Smart Importer)
